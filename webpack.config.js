...
const { getCurrentWorkingDirectory } = require( './config/process' );
...
module.exports = function( webpackEnv ) {
	...
	return {
		...
		// These are the "entry points" to our application. This means they will be the "root" imports that are included in JS bundle.
		entry:
		...
		resolve: {
			...
			alias: {
				...
				// Pretend lodash-es is lodash
				"lodash-es": "lodash",
			},
			plugins: [
				// Adds support for installing with Plug'n'Play, leading to faster installs and adding guards against forgotten dependencies and such.
				PnpWebpackPlugin,
				/*
				...
				{ parser: { requireEnsure: false } },
				{
					...
					oneOf: [
						...
						// Process application JS with Babel. The preset includes JSX, Flow, TypeScript, and some ESnext features.
						{
							...
							include: [ paths.appSrc, /node_modules[/\\](?=(yoast-components|yoastseo|@yoast)[/\\]).*/ ],
						...
						// ** STOP ** Are you adding a new loader? Make sure to add the new loader(s) before the "file" loader.
					],
				},
			],
		},
		plugins: [
			...
			new ReactRefreshWebpackPlugin( {
				overlay: {
					...
					// Since we ship a custom dev client and overlay integration, the bundled socket handling logic can be eliminated.
			...
			new WorkboxWebpackPlugin.InjectManifest( {
				...
				// Bump up the default maximum size (2mb) that's precached, to make lazy-loading failure scenarios less likely.
			...
			new ForkTsCheckerWebpackPlugin( {
				...
				reportFiles: [
					/*
					 ...
					 '../cra-template-typescript/template/src/App.tsx' otherwise.
					 */
					"../**/src/**/*.{ts,tsx}",
					"**/src/**/*.{ts,tsx}",
					"!**/src/**/__tests__/**",
